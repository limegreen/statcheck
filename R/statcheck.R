statcheck <-
  function(x,
    ...
    # order dataframe by location of the test in the article
    ...
    if (nrow(Res) > 0) {
      ...
      if (OneTailedTests == FALSE) {
        ...
        if (any(OneTail[!is.na(OneTail)] == TRUE &
          ...
          message(
            "\n Check for one tailed tests. \n \n Some of the p-value inconsistencies might in fact be one-tailed tests. It is recommended to check this in the actual paper or text. Check if the p-values would also be inconsistent if the test is indeed one-sided by running statcheck again with 'OneTailedTests' set to TRUE. \n "
          )
        }
      }
      ...
      if (OneTailedTxt == TRUE) {
        ...
        Res$Error[!((Res$Statistic %in% c("F", "Chi2", "Q")) & Res$df1 > 1) &
          Res$OneTailedInTxt == TRUE & 
            Res1tailed$Error == FALSE] <- FALSE
        
        Res$DecisionError[!((Res$Statistic %in% c("F", "Chi2", "Q")) & Res$df1 > 1) &
          Res$OneTailedInTxt == TRUE &
      ...
      CorrectRound <- correct_rounding(Res)
      ...
      # this is relevant, because it sometimes happens that statcheck extracts fewer p values
      # than statcheck results. For instance in cases when a p value appears to be greater than 1.
